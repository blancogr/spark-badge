...
public class BibleWordCount {
  ...
  private static void run(final JavaSparkContext sc, final String input, final String stopWordsIn, final String output) {
    HashSet<String> stopWords = new HashSet<>(sc.textFile(stopWordsIn).collect());
    Broadcast<HashSet<String>> broadcast = sc.broadcast(stopWords);

    sc
      .textFile(input)
      .mapToPair(w -> new Tuple2<>(w, 1))
      .filter(tuple -> !broadcast.getValue().contains(tuple._1()))
